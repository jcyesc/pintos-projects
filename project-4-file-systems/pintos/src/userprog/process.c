...
process_exit (void)
{
  ...
  for (i = STARTING_RANGE_FILE_DESCRIPTORS; i < MAX_FILE_DESCRIPTORS; i++)
    {
      if (cur->file_descriptors[i].type == FILE_INODE)
        {
          struct file *current_file = cur->file_descriptors[i].file;
          if (current_file != NULL)
            file_close(current_file);
        }
      else
        {
          struct dir *current_dir = cur->file_descriptors[i].dir;
          if (current_dir != NULL)
...
/* Tokenizes the command line. It takes the command line and when it
