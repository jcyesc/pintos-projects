...
{
  return inode_create (sector, entry_cnt * sizeof (struct dir_entry), DIRECTORY_INODE);
}
...
dir_remove (struct dir *dir, const char *name) 
{
  ...
  if (inode_type(inode) == DIRECTORY_INODE)
    {
      ...
      if (e.in_use && strcmp (".", e.name) && strcmp ("..", e.name))
        {
          ...
          //if (inode != NULL && inode_type(inode) == DIRECTORY_INODE)
          if (inode != NULL)
            {
