...
static void tokenize_command_line(const char *command_line, struct list *ptr_list);
static void release_memory_elem_list (struct list *ptr_list);

/* Functions to set up the stack with the command line parameters. */
static uint32_t set_parameters_stack(struct list *ptr_list, uint32_t address_parameters[]);
...
static void release_resources(struct thread *t);

/* Remove the supplemental table and map_info entries. */
static void remove_supplemental_table_entry (struct hash_elem *e, void *aux);
static void remove_mmap_info_table_entry(struct hash_elem *e, void *aux);
...
static void
remove_supplemental_table_entry(struct hash_elem *e, void *aux UNUSED)
{
  struct supplemental_page *sp = hash_entry (e, struct supplemental_page, hash_elem);
  if (sp->loaded)
    {
      pagedir_clear_page(thread_current()->pagedir, (void *) sp->virtual_user_address);
      frame_free_frame (sp->kpage);
    }
  free (sp);
}

static void
remove_mmap_info_table_entry(struct hash_elem *e, void *aux UNUSED)
{
  struct mmap_info *mmap = hash_entry (e, struct mmap_info, hash_elem);
  dump_and_release_mmap(mmap, thread_current());
  free (mmap);
}
...
/* It releases all the resources associated with this thread. */
static void
release_resources(struct thread *t)
{
  ...
  struct process_monitor *parent_monitor = get_process_monitor(t->parent_tid);
  ...
  // TODO more children
  remove_dead_children(t->tid);

  /* Releasing the memory for the mmap hash table and closing the files. In addition
   * to this, it removes the supplemental tables associated with the memory map. */
  hash_clear(&t->mmap_ids, remove_mmap_info_table_entry);

  /* Removing all the supplemental tables. */
  hash_clear(&t->supplemental_pages, remove_supplemental_table_entry);
}
...
static bool load_segment (struct thread *t, off_t ofs, uint8_t *upage,
...
*/
static void
tokenize_command_line(const char *command_line, struct list *ptr_list)
{
  char *token, *save_ptr;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Init supplemental table and mmap table. */
  hash_init (&t->supplemental_pages, supplemental_page_hash, supplemental_page_less, NULL);
  hash_init (&t->mmap_ids, mmap_hash, mmap_hash_less, NULL);
          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (!load_segment (t, file_page, (void *) mem_page,
...
static bool
load_segment (struct thread *t, off_t offset, uint8_t *upage,
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      struct supplemental_page *sup_page = malloc (sizeof (struct supplemental_page));
      sup_page->type = EXECUTABLE;
      sup_page->read_bytes = page_read_bytes;
      sup_page->zero_bytes = page_zero_bytes;
      sup_page->virtual_page_number = pg_no (upage); /* The offset is eliminated from upage. */
      sup_page->virtual_user_address = (uintptr_t) upage;   /* The offset is ZERO. */
...
/* Sets the command line parameters at the top of the stack. Start setting the last parameter,
   ...
   It returns the address in the stack where the first parameter is.

   Note:  The size of address_parameters has to be equal to size of ptr_list.
 */
static uint32_t
set_parameters_stack(struct list *ptr_list, uint32_t address_parameters[])
{
      ...
      for (j = strlen(param); j >= 0; j--)
        {
          ...
          put_user ((uint32_t *) stack_address, param[j]);
        }
...
set_word_align(uint32_t stack_address)
{
  ...
  while (num_chars % STACK_SIZE_BYTES != 0)
    {
      stack_address--;
      put_user((uint32_t *) stack_address, 0);
      num_chars++;
    }
}
...
setup_stack (void **esp, struct list *ptr_list)
{
  ...
  kpage = frame_get_frame (PAL_USER | PAL_ZERO, upage);
  ...
  if (kpage != NULL)
    {
      ...
      hash_insert(&thread_current()->supplemental_pages, &sup_page->hash_elem);
      ...
      if (success)
        {
          ...
          /* Setting the parameters in the stack. */
          uint32_t stack_address = set_parameters_stack(ptr_list, address_parameters);
