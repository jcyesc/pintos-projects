...
#define STACK_SIZE (1 << 23)               /* Stack size 8 MB*/
#define CODE_SEGMENT 0x08048000            /* Code segment start */
...
/* Returns true if the VADDR is between PHYS_BASE and the Code segment */
static inline bool
is_valid_user_access_vaddr (const void *vaddr)
{
  return is_user_vaddr(vaddr) && ((uintptr_t) vaddr >= CODE_SEGMENT);
}

/* The stack can grow maximum 8MB */
static inline bool
is_user_stack_vaddr (const void *vaddr)
{
  return is_user_vaddr(vaddr) && (uintptr_t) vaddr >= ((uintptr_t) PHYS_BASE - STACK_SIZE);
}
